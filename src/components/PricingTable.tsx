...
const PricingTable: React.FC<PricingTableProps> = ({ accountType }) => {
  ...
  const futuresFeatures: PlanFeature[] = [
    ...
    },
    {
      ...
      values: Array(6).fill(
        "Max time limit: 3 months on evaluation, unlimited in live"
      ),
    },
    {
    ...
    },
    {
      ...
      values: Array(6).fill(
        "Keep 75% (90% with upgrade), first withdrawal whenever you want"
      ),
    },
    {
            ...
            <tr>
              ...
              {accountType === "forex" ? (
                accountSizes.map((size, index) => (
                  <th
                    key={index}
                    className="py-8 px-6 text-center font-bold text-xl border-b border-purple-500/20"
                  >
                    {size}
                  </th>
                ))
                ...
                {fees.map((fee, index) => (
                  <td key={index} className="py-2 px-4 text-center">
                    <div className="flex flex-col items-center justify-center mb-6">
                      <Button className="bg-[#4C6EF5] hover:bg-[#4C6EF5]/90 text-white w-full max-w-[140px] rounded-xl mb-1 py-6">
                        Get Plan
                      </Button>
